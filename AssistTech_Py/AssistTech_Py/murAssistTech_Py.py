...
def main():
    ...
    while True:
        ...
        if is_first_frame:
            largest_area = 0
            gaussian_blurred = cv2.GaussianBlur(frame, (5, 5), 0) # Kernel size de?i?ebilir noise robust için
            ...
            for i, cnt in enumerate(contours):
                ...
                if not ((500 < area < 3000) or area > tmp_size / 8): # area aral??? çözülmesi gerekiyor
                    if largest_area < area:
                        x, y, w, h = cv2.boundingRect(cnt)
                        if w > 100 and h > 100: # kare bulmam?z ve belli bir uzunluktan fazla olmas? çözülmesi gerekiyor
                            bbox = (x, y, w, w)
                            previous_y = y
                            previous_x = x
                            largest_area = area
            ...
            if ok:
                ...
                if (previous_x - 3 < bbox[0] < previous_x + 3) and (previous_y - 3 < bbox[1] < previous_y + 3): # direkt e?it mi? yoksa bir aral??a göre mi??
                    is_first_frame = True
                else:
                    previous_x = bbox[0]
                    previous_y = bbox[1]
